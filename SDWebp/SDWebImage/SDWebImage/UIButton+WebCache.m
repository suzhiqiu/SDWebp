...
- (void)sd_setImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    //解决线上问题的 更换tabbar图片 把快报 改为 理财返还。  会每次优先显示默认图片快报，接着才显示理财返还
    NSString *key = [[SDWebImageManager sharedManager] cacheKeyForURL:url];
    UIImage *lastPreviousCachedImage=nil;
    if(key)
    {
        lastPreviousCachedImage= [[SDImageCache sharedImageCache] imageFromDiskCacheForKey:key];
    }
    
    [self setImage:lastPreviousCachedImage? lastPreviousCachedImage:placeholder forState:state];
